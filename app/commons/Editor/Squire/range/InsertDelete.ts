...
import { SQUIRE_IMG_ID_KEY } from '../Constants';
import { Squire } from '../Editor';
import { getNextBlock, getPreviousBlock, isEmptyBlock } from '../node/Block';
import { isContainer, isInline, isLeaf } from '../node/Category';
import {
  fixContainer,
  fixCursor,
  mergeContainers,
...
import { detach, getLength, getNearest } from '../node/Node';
...
const insertNodeInRange = (range: Range, node: Node): void => {
  ...
  if (startContainer instanceof Text) {
    ...
    if (startOffset === startContainer.length) {
      ...
      startOffset = Array.from(children).indexOf(startContainer as ChildNode);
    }
...
const deleteContentsOfRange = (
  ...
  // FIX nasz, jezeli w fragmencie do usuniecia sa obrazki w trakcie uploadu to anulujmy im upload
  frag.querySelectorAll(`img`).forEach((img) => {
    const id = img.getAttribute(SQUIRE_IMG_ID_KEY);

    if (id) {
      self._config.uploadImageCancel(id);
    }
  });
