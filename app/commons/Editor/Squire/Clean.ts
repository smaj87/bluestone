...
import { createElement, detach, empty, replaceWith } from './node/Node';
...
> = {
  ...
  'font-family': {
    regexp: notWS,
    replace(classNames: { fontFamily: string }, family: string): HTMLElement {
...
const replaceWithTag = (tag: string) => (node: HTMLElement, parent: Node) => {
        ...
        if (startsWithWS) {
          ...
          while ((sibling = walker.previousPONode())) {
            if (
              ...
              (sibling instanceof Text && notWS.test(sibling.data))
            ) {
              break;
            }
...
export { cleanTree, cleanupBRs, escapeHTML, isLineBreak, removeEmptyInlines };
