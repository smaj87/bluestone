...
const setClipboardData = (
  ...
  // Tutaj dodajemy detail w event, bo tworzymy customowy clipboard dzieki któremu ustawiamy te wartosc do schowka w Edytorze w custom context menu
  const clipboardData = (event.clipboardData ||
    (event as unknown as CustomEvent).detail.clipboardData)!;
  const { body } = document;
...
const extractRangeToClipboard = (
  ...
  if (
    ...
    if (removeRangeFromDocument) {
      contents = deleteContentsOfRange(self, range, root);
    } else {
...
// --- obsługa wklejania danej wartości do HTMLa ---
// onPaste jest glownym handlerem ktory pozniej zaleznie od itema odpala poszczegolne handlery takie jak np. image -> onDrop, onDragOver
// WAŻNE BRAK obsługi Dragover które jest prawdziwym handlerem image paste
...
const _onPaste = function (this: Squire, event: ClipboardEvent): void {
  ...
  const choosePlain: boolean | undefined =
    this._isShiftDown || this._config.isInsertOnlyPlainText;
  let fireDrop = false;
  let hasRTF = false;
  let hasImage = false;
...
// eslint-disable-next-line func-names
const _onDragOver = function (this: Squire, _event: DragEvent): void {
  // event.preventDefault();
};
...
// eslint-disable-next-line func-names
const _onDrop = function (this: Squire, _event: DragEvent): void {
  // event.preventDefault();
  //
