...
import { getBackgroundColor } from 'commons/Editor/selectors';
...
import {
  DROPDOWN_POPUP_ID,
  DROPDOWN_TARGET_ID,
  EDITOR_BG_COLORS_MENU_SUBMENU_ID,
} from './constants';
...
const ColorsDropdown: FC<Props> = ({ editorId }) => {
  ...
  const submenuId = useMemo(
    () => `${EDITOR_BG_COLORS_MENU_SUBMENU_ID}${editorId}`,
    [editorId],
  );
      ...
      desktop={
        <>
          <Button
            ...
            title={t('setColor')}
          >
            <CurrentColorInfoStyled $bg={currentColor || TEXT_COLORS.white} />
